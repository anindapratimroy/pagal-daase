...
console.log('Generating Schema.org JSON-LD Graph...');
...
console.log('Generating Semantic Pre-rendered HTML Crawler Directory...');
...
for (const area of fb.RESEARCH_AREAS) {
  htmlDirectory += `
        <div class="seo-research-area">
          ...
          <div class="seo-research-detail">
            ${(area.full_description || '').split('\n\n').map(p => `<p>${p.trim()}</p>`).join('')}
          </div>
        </div>`;
}
...
export { comprehensiveJsonLd, htmlDirectory };

// If executed directly, run injection test
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  console.log('Testing generate-seo output size:');
  console.log('JSON-LD Graph Items:', comprehensiveJsonLd['@graph'].length);
  console.log('HTML Directory Length:', htmlDirectory.length, 'characters');
}
