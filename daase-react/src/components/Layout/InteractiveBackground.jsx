...
const InteractiveBackground = () => {
  ...
  useEffect(() => {
    ...
    const getIsTouchDevice = () => {
      ...
      return (hasCoarse || hasTouch) && !hasFine;
    };

    let isTouch = getIsTouchDevice();

    const getCounts = () => {
      const W = window.innerWidth;
      if (W < 768) {
        return { stars: 120, nebulas: 3, auroras: 1, lineDist: 60, lineCheck: 25 };
      }
      if (W < 1024) {
        return { stars: 175, nebulas: 4, auroras: 2, lineDist: 75, lineCheck: 45 };
      }
      return { stars: 320, nebulas: 7, auroras: 3, lineDist: 95, lineCheck: 100 };
    };
    ...
    const onMove = e => {
      // Ignore synthetic mouse events generated by touch
      if (isTouch || (e.sourceCapabilities && e.sourceCapabilities.firesTouchEvents)) return;
    ...
    const onTouchStart = () => {
      // Instantly clear mouse coordinates on touch so cursor aura never freezes on mobile
      mouseRef.current.x = -9999;
      mouseRef.current.y = -9999;
    };

    // Click = gravitational ripple wave (only for true mouse clicks, not touch taps)
    const onClick = e => {
      if (isTouch || (e.sourceCapabilities && e.sourceCapabilities.firesTouchEvents)) return;
    ...
    const animate = () => {
      ...
      // ── Cursor aura (only when using a physical mouse/pointer) ──
      ...
      // ── Constellation lines (optimized count and distance for mobile/tablet) ──
      ctx.lineWidth = 0.5;
      const chk = Math.min(stars.length, counts.lineCheck);
      const lineDistSq = counts.lineDist * counts.lineDist;
      for (let i = 0; i < chk; i++) {
        let c = 0;
        for (let j = i + 1; j < chk && c < 3; j++) {
          ...
          if (ds < lineDistSq) {
            ...
            const la = (1 - d / counts.lineDist) * LINE_ALPHA * Math.min(stars[i]._a, stars[j]._a);
